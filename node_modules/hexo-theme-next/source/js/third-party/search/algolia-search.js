...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const formatHits = data => {
    ...
    // 2. 获取高亮的片段 (Snippet)
    const snippet = data._snippetResult?.contentStrip?.value 
                  || data._highlightResult?.contentStrip?.value 
                  || '';

    let result = `<li><a href="${data.permalink}" class="search-result-title">${title}</a>`;
  
    if (snippet) {
      result += `<a href="${data.permalink}"><p class="search-result">${snippet}...</p></a></li>`;
    }
    return result;
  };
  ...
  const searchAlgolia = async (searchText, page = 0) => {
    ...
    const result = await client.search({
      requests: [{
        ...
        attributesToRetrieve : ['permalink', 'contentStrip'], // 增加 contentStrip
        attributesToSnippet  : ['contentStrip:30'],
        attributesToHighlight: ['title', 'contentStrip'],     // 确保对正文执行高亮处理
